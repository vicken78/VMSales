        ...
        public static List<PurchaseOrderModel> getPurchaseOrderRelationship()
        {
            using (var connection = new SQLiteConnection(getConnectionString()))
            {
                ...
                List<PurchaseOrderModel> list = connection.Query<PurchaseOrderModel, PurchaseOrderDetailModel, PurchaseOrderModel>
                        (@"SELECT po.purchase_order_id, po.supplier_fk, po.invoicenum, po.purchasedate, 
                         pod.purchase_order_id as ID,  pod.lotnum, pod.lotcost, pod.lotqty,  
                      pod.lotname, pod.lotdescription, pod.salestax, pod.shippingcost from purchase_order po 
                      INNER JOIN purchase_order_detail pod on po.purchase_order_id = pod.purchase_order_id", (order, orderDetail) =>
                      {
                          if (!orderDictionary.TryGetValue(order.purchase_order_id, out PurchaseOrderModel orderEntry))
                          {
                              orderEntry = order;
                              orderEntry.OrderDetails = new List<PurchaseOrderDetailModel>();
                              orderDictionary.Add(orderEntry.purchase_order_id, orderEntry);
                          }
